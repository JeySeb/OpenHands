...
'''
    ...
    if uses_external_api:
        # Convert default_headers dict to Python dict literal string
        headers_repr = repr(default_headers) if default_headers else '{}'
...
def _resolve_headers(custom_headers: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    """
    Resolve headers by merging default headers with custom headers.
    ...
    Returns:
        Resolved headers dictionary with environment variables substituted
    ...
    Examples:
        ...
        headers = _resolve_headers({{"X-Request-ID": "12345"}})

        # Environment variable substitution
        # If default_headers = {{"Authorization": "Bearer ${{API_TOKEN}}"}}
        # and os.environ["API_TOKEN"] = "secret123"
    ...
    for key, value in default_headers.items():
        # Check for environment variable pattern: ${{ENV_VAR}}
        if "${{" in value:
    ...
    if custom_headers:
        resolved.update(custom_headers)
...
def _make_request(
    ...
    Args:
        ...
        headers: Optional custom headers to include (merged with default headers)
        mcp_tool: Name of the calling tool for metadata
    ...
    Examples:
        ...
        result = _make_request('GET', '/api/users')
    ...
    request_args = {{"timeout": {api_timeout}, "headers": resolved_headers}}
    payload_for_logging = {{}}
    ...
    logger.debug(f"HTTP {{method.upper()}}: {{resolved}} {{payload_for_logging}} Headers: {{resolved_headers}}")
...
def _get_json(
    ...
    Args:
        ...
        mcp_tool: Tool name for metadata

    Example:
        result = _get_json('/api/users', params={{"page": 1}}, headers={{"X-Request-ID": "123"}})
    """
    return _make_request('GET', endpoint, params=params, headers=headers, mcp_tool=mcp_tool)
...
def _post_json(
    ...
    Example:
        result = _post_json('/api/users', data={{"name": "John"}}, headers={{"Content-Type": "application/json"}})
    """
    return _make_request('POST', endpoint, json_data=data, headers=headers, mcp_tool=mcp_tool)
...
def _put_json(
    ...
    Example:
        result = _put_json('/api/users/1', data={{"name": "John Updated"}})
    """
    return _make_request('PUT', endpoint, json_data=data, headers=headers, mcp_tool=mcp_tool)
...
def _delete_json(
    ...
    Example:
        result = _delete_json('/api/users/1', headers={{"X-Confirm": "true"}})
    """
    return _make_request('DELETE', endpoint, params=params, headers=headers, mcp_tool=mcp_tool)
'''
